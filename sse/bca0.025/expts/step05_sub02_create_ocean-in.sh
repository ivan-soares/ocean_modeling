...
#
	...
	echo
	echo " +++ Starting step05 s.r. 02: prepare ocean.in file for $mytitle +++"
	echo
	...
	sed -e "s|INPUTS|$inpdir|g" -e "s|OUTPUTS|$outdir|g" \
	   $exptdir/ocean_bca0.025_no_sst.in >& ocean_${expt}.in

	#sed -e "s|INPUTS|$inpdir|g" -e "s|OUTPUTS|$outdir|g" \
	#  $exptdir/ocean_npo0.08_gfs.in >& ocean_${expt}.in
	...
	#   Lm == 401           ! Number of I-direction INTERIOR RHO-points
	#   Mm == 291           ! Number of J-direction INTERIOR RHO-points
	#    N == 30            ! Number of vertical levels

	let nxx=$nx-2
	let nyy=$ny-2

	#sed -i "/ Lm == /      c\  Lm == $nxx "        ocean_${expt}.in
	#sed -i "/ Mm == /      c\  Mm == $nyy "        ocean_${expt}.in
	...
	sed -i "/ LcycleRST == /  c\  LcycleRST == $lcycle_rst         ! T/F " ocean_${expt}.in
