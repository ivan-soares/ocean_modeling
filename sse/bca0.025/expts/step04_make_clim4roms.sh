...
#
	...
	if [ -e  grid_r.nc ]; then rm grid_r.nc; fi
        if [ -e weights_r.nc ]; then rm weights_r.nc; fi
	cp $here/d-interp/depths_${ogcm}_${domain_roms}.nc grid_r.nc

	$cdo genbil,grid_r.nc $ogcmfile weights_r.nc
	$cdo remap,grid_r.nc,weights_r.nc $ogcmfile remaped_${ogcm}_$today.nc
	rm grid_r.nc weights_r.nc
